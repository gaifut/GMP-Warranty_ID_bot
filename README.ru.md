...
1. Клонируйте этот репозиторий. ```git clone git@github.com:gaifut/GMP-Warranty_ID_bot.git```
...
5. Создайте файл sensitive.py, в котором укажите адреса файлов с таблицами. Вы также можете не создавать отдельный файл, проект я делал давно и на Windows, поэтому оставил именно эту логику.
   ...
   ```
6. Внесите изменения в файл definitions.py:
...
7. Запустите проект из файла GMP_W_B_main 1.3.py (в VSCode горячая клавиша запуска F5). После этого ваш телеграм бот должен начать работать.
