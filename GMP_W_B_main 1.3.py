...
sa = gspread.service_account(filename = s.Google_sheets_API_details)
...
credentials = ServiceAccountCredentials.from_json_keyfile_name(s.Google_sheets_API_details, scope)
...
@dp.callback_query_handler(lambda call: call.data in ['warranty', 'spareparts_sales'])
...
async def receive_ID_info (message: types.Message, state: FSMContext):
    try:   
        ...
        session_start_time = data_receive_ID_info_W_handler_time.get("session_start_time")
        print (warranty_state_dictionary)
        wks.append_row([warranty_state_dictionary['user_no'], warranty_state_dictionary['provide_ID'], warranty_state_dictionary['session_start_time'],warranty_state_dictionary['query_count']])
        if time.time() - session_start_time > d.session_timeout:
            await message.answer(d.session_timeout_message)
            await state.finish()
            return
...
async def receive_sales_ID_info(message: types.Message, state: FSMContext):
    try:
        ...
        wks.append_row([spareparts_sales_dictionary['user_no'], spareparts_sales_dictionary['provide_ID'], spareparts_sales_dictionary['session_start_time'],spareparts_sales_dictionary['query_count']])
    except KeyError:
        await message.answer(d.error_message)
    except Exception as e:
        await message.answer(f"An error occurred: {e}")
